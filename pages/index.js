          ...
          transitionLeaveTimeout={500}>
            ...
            {decendingMessageIds.map( id => {
              ...
              return <li
                ...
                <p>
                  {message.bufferLength} changes performed
                  <br/>
  ...
  subscribeToSalesforceMessages = () => {
    const connect = () => {
      ...
      this.eventSource.addEventListener("salesforce", event => {
        ...
        if (!this.transactionBuffer[id]) this.transactionBuffer[id] = [];
        this.transactionBuffer[id].push(message);
        const bufferLength = this.transactionBuffer[id].length;
        setTimeout(() => {
          if (bufferLength === this.transactionBuffer[id].length) {
            delete this.transactionBuffer[id];
